...
func ingressRoleCheckNginx(clientset *kubernetes.Clientset) error {
	...
	for _, rule := range role.Rules {
		if contains(rule.APIGroups, requiredApiGroup) &&
			...
			fmt.Printf("Role %s in namespace %s does not have the required permissions to run SV with Nginx\n", roleName, workingNs)
		}
	}
	return nil
}
...
func ingressRoleCheckIstio(clientset *kubernetes.Clientset) error {
	...
	for _, rule := range role.Rules {
		if contains(rule.APIGroups, requiredApiGroup) &&
			...
			fmt.Printf("Role %s in namespace %s does not have the required permissions to run SV with Istio\n", roleName, workingNs)
		}
	}
	return nil
}
